...
class StudentSkillController extends Controller
{
    ...
    public function view($id)
    {
        $course = Course::with([
            ...
            'settings'
        ])->whereIn('type', [1, 2])->findOrFail($id);
        ...
        // لاگ اطلاعات دوره
        Log::info('=== دوره مهارتی ===', [
            'course_id' => $course->id,
            'length' => $course->length,
            'sessions_length' => $course->sessions_length,
                ...
                if ($sessions->isEmpty()) {
                    ...
                    if ($member == 1) {
                        ...
                        if (isset($course->length) && $course->length > 0 && isset($course->sessions_length) && $course->sessions_length > 0) {
                            ...
                            if ($periodPerSession <= 0)
                                $periodPerSession = 1;
                        }

                        // لاگ periodPerSession
                        Log::info('periodPerSession', ['value' => $periodPerSession]);

                        $unlockedCount = min($totalSessionsCount, floor($diffInDays / $periodPerSession) + 1);

                        // لاگ unlockedCount
                        Log::info('unlockedCount', [
                            'value' => $unlockedCount,
                            'totalSessionsCount' => $totalSessionsCount,
                            'calculation' => "floor({$diffInDays} / {$periodPerSession}) + 1 = " . floor($diffInDays / $periodPerSession) + 1,
                        ]);

                        if ($unlockedCount < $totalSessionsCount) {
                            $nextUnlockTime = Carbon::parse($courseUser->created_at)->addDays($unlockedCount * $periodPerSession);
                            Log::info('nextUnlockTime محاسبه شد', [
                                'nextUnlockTime' => $nextUnlockTime,
                                'addDays' => $unlockedCount * $periodPerSession,
                            ]);
                        } else {
                            Log::info('همه جلسات باز شده‌اند');
                        }
