        ...
        <div class="course-actions-bar">
            ...
            @if(isset($nextUnlockTime) && $nextUnlockTime && $member == 1)
                <div class="info-badge topic-badge">
                    ...
                    <span id="timerDisplay" data-unlock="{{ $nextUnlockTime->utc()->toIso8601String() }}">
                        --:--:--
                    </span>
                </div>
            @endif
...
@section('js')
    <script>
        ...
        function changeSession(element, sessionData) {
            ...
            const pdfUrl = sessionData.file;
            const title = sessionData.name;
            const number = sessionData.number;
            const description = sessionData.text || '';
            const canQuestion = sessionData.can_question;
            const canHomework = sessionData.can_homework;
            const canReport = sessionData.can_report;

            currentSessionId = sessionId;
            currentPdfUrl = pdfUrl;
            currentSessionTitle = title;
            currentSessionNumber = 'جلسه ' + number;
            currentDescription = description;

            const sessionNumberDisplay = document.getElementById('sessionNumberDisplay');
            if (sessionNumberDisplay) sessionNumberDisplay.textContent = number || '-';

            const sessionNameDisplay = document.getElementById('sessionNameDisplay');
            if (sessionNameDisplay) sessionNameDisplay.textContent = title || 'هیچ جلسه‌ای انتخاب نشده است';

            const pdfViewer = document.getElementById('pdfViewer');
            if (pdfViewer) {
                if (pdfUrl) {
                    const fullUrl = pdfUrl.startsWith('http') ? pdfUrl : '/files/session' + pdfUrl;
                    pdfViewer.outerHTML = `
                        <object id="pdfViewer" data="${fullUrl}" type="application/pdf" width="100%" height="550px">
                            <object width="100%" height="550" data="https://docs.google.com/gview?embedded=true&url=${fullUrl}"></object>
                        </object>
                    `;
                } else {
                    pdfViewer.innerHTML = `
            ...
            if (pdfOpenBtn) {
                if (pdfUrl) {
                    const fullUrl = pdfUrl.startsWith('http') ? pdfUrl : '/files/session' + pdfUrl;
                    pdfOpenBtn.setAttribute('href', fullUrl);
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ===== تایمر شمارش معکوس (رفع خطای تاریخ) =====
            const timerDisplay = document.getElementById('timerDisplay');
            if (timerDisplay) {
                const unlockStr = timerDisplay.dataset.unlock;
                if (!unlockStr || unlockStr.trim() === '') {
                    timerDisplay.textContent = 'زمان نامشخص';
                    return;
                }
                // تاریخ به صورت ISO با offset است، مستقیماً parse می‌شود
                const unlockTime = new Date(unlockStr).getTime();
                if (isNaN(unlockTime)) {
                    timerDisplay.textContent = 'خطا در تاریخ';
                    return;
                }
